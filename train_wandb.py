...
def main():
	config = Config()
	wandb.init(project="221031", name="90_025_5s15d")
	...
	for j in range(num_updates):
		...
		if np.mean(episode_rewards) >= best_rewards:
			best_rewards = np.mean(episode_rewards)
			torch.save(actor_critic.state_dict(), os.path.join(save_path, f"best-{j:d}-{best_rewards:.3f}.pt"))
		...
		if j % config.training.log_interval == 0 and len(episode_rewards) > 1:
			...
			wandb.log({'misc/nupdates': j, 'misc/total_timesteps': total_num_steps,
							   'fps': int(total_num_steps / (end - start)), 'eprewmean': float(np.mean(episode_rewards)),
							   'loss/policy_entropy': dist_entropy, 'loss/policy_loss': action_loss,
							   'loss/value_loss': value_loss})
